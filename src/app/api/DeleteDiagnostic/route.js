...
export async function DELETE(request) {
  try {
    ...
    return new Response(JSON.stringify({ message: 'Diagnóstico y pruebas eliminados' }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error('Error al eliminar el diagnóstico:', error);
    return new Response(JSON.stringify({ error: 'Error al eliminar el diagnóstico' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  } 
}
